...
class SafetyGymEnv():
    def __init__(self, env_id, size=(64, 64), action_repeat=5, bit_depth=5, device='cpu'):
    ...
    def reset(self):
        ...
        return _images_to_observation(image, self._bit_depth).to(self.device)
    ...
    def step(self, action):
        ...
        image = self._env.task.render(width=self._size[0], height=self._size[1], 
                                      mode='rgb_array', camera_name=self._camera_name, cost={})
        obs = _images_to_observation(image, self._bit_depth).to(self.device)
